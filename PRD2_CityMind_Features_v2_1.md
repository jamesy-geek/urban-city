...
**Scope**
CityMind AI Chatbot — 4 New Features
**Version**
...
This document defines four new features to be added to the CityMind AI Chatbot (Section 5 of PRD 2). These features extend the existing chatbot at /citizen/chat without modifying any other part of the Citizen Dashboard. All four features are additive — they do not replace or alter existing chatbot behavior.
...
Feature 4 — Multi-Level Complaint Ticketing  :  Citizens can raise complaint tickets via chat. Tickets route first to the Municipality, escalate to the District Commissioner after 7 days if unresolved, and maintain statuses (open, escalated, resolved ticket). Chatbot can fetch historical ticket data.
...
**§4  ****Feature 4 — Multi-Level Complaint Ticketing System**

## **4.1  What This Feature Does**
Citizens can raise actionable complaint tickets through the CityMind Chatbot. The system incorporates a multi-level government routing workflow. Tickets first arrive at the 1st level (Municipality Corporation). If unresolved after a defined SLA timeframe (7 days), tickets are automatically escalated to the 2nd level (District Commissioner). Citizens can query the chatbot for their history of tickets, and statuses are kept sequentially as "open", "escalated", or "resolved ticket".

## **4.2  Trigger Conditions & Intents**
CityMind triggers the ticketing flow when the citizen expresses the intent to:
- "Raise a complaint", "Report an issue", "File a ticket"
- Or asks about history: "What are my tickets?", "Show my ticket history", "Check complaint status"

## **4.3  Ticket Creation Flow**
1. Chatbot asks for basic complaint details (Type of issue, Location, Description).
2. It generates a ticket and returns a confirmation to the user with a Ticket ID.
3. The ticket is immediately routed to the 1st Level (Municipality Corporation).
4. Default status of the newly created ticket is "open".

## **4.4  Ticket Escalation & Statuses**
- **open**: Ticket is created and with the Municipality Corporation.
- **escalated**: If 7 days pass without resolution, the ticket auto-routes to the 2nd Level (District Commissioner) and its status becomes "escalated".
- **resolved ticket**: When the issue is fixed by government officials, the status updates to this exact phrase.

## **4.5  Ticket History Interface & Storage**
- Real-time ticket records are stored in the citizen's profile and displayed in the new "Tickets Document" interface (part of the Citizen Dashboard).
- The Chatbot has access to fetch these historical records when queried by the citizen.

**§5  ****Backend Changes & New Endpoints**

## **5.1  Updated System Prompt (Full Replacement)**
...
[NEW] TICKETING SYSTEM: If the user wishes to raise a complaint, collect issue type, location, and description, then inform them that the ticket will be registered with the Municipality Corporation, and will escalate to the District Commissioner if unresolved in 7 days. If the user asks about ticket history, display their past tickets with their statuses ('open', 'escalated', 'resolved ticket').
...
## **5.2  New Letter Generation Endpoint**
**POST /api/v1/citizen/generate-letter**
...
## **5.3  Letter PDF Retrieval Endpoint**
...
## **5.4  New Ticketing Endpoints**
**POST /api/v1/citizen/tickets**
Body: { session_id, issue_type, location, description }
Response: { ticket_id, status, assigned_level, created_at }

**GET /api/v1/citizen/tickets**
Response: Array of user's tickets to serve chatbot history queries.
...
## **6.2  PDF Download Card Component**
...
## **6.4  Permission Hierarchy Card Component**
...
## **6.5  Ticket History Card Component**
Location: Rendered in chat or as a link to the "Tickets Document" view.
- Component: <TicketHistoryCard />
- Shows Ticket ID, Issue, Date, and current Status Badge (Open, Escalated, Resolved Ticket).


**§7  ****Testing & Acceptance Criteria**

## **7.1 Feature 1 — PDF Generation**
...
## **7.3 Feature 3 — Permission Recommendation**
...
## **7.4 Feature 4 — Complaint Ticketing**
- Chatbot can guide a user to raise a new ticket successfully.
- Chatbot retrieves historical tickets when asked.
- Ticket statuses strictly follow the 'open', 'escalated', 'resolved ticket' taxonomy.
- Escalation business logic (7 days) is properly planned for the backend worker.


**§8  ****Constraints & What Must Not Change**
