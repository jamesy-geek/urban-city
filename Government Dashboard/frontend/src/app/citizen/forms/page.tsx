...
export default function CivicFormsPage() {
  ...
  const handleSubmit = async () => {
    const payload = {
      type: FORMS.find(f => f.id === selectedForm)?.title || selectedForm,
      applicant: "Citizen Submission", // Placeholder, ideally from input
      date: new Date().toISOString().split('T')[0],
      route: "Mysuru Central",
      formData: { selectedForm, step } // Simplified for now
    };
